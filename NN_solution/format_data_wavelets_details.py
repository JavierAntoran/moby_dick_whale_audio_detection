...
wavelet = 'sym5'
level = 4
aprox_idx = level - np.array([1,2,3,4])
...
def get_wfb_swt_dct(x):
    ...
    approx = ac_dc[aprox_idx, 1] # actually using details because index is one
...
ready_data = np.zeros((sounds.shape[0], 160, 60))  #sum(num_cepst)=30
...
np.save('data/processed_data_swt_details.npy', ready_data)